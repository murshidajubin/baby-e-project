...
const OrderPage = () => {
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <h2 className="text-3xl font-bold mb-6 text-center text-gray-800"> 🛍️ Your Orders</h2>
      {orders.length === 0 ? (
          ...
          {orders.map((order, index) => (
            <div key={index} className="border p-6 rounded-lg shadow-lg bg-white">
              <div className="flex justify-between items-center">
                ...
                <p
                  className={`font-bold ${
                    order.paymentMethod === "Cash on Delivery" ? "text-green-600" : "text-blue-600"
                  }`}
                >
                  {order.paymentMethod}
                </p>
              </div>
              <p className="text-gray-600 font-medium">Total: ₹{order.total.toFixed(2)}</p>
              ...
              {order.shippingAddress ? (
                <div className="mt-4 p-4 border rounded-lg bg-gray-100">
                 ...
                 <p className="text-red-500 font-semibold mt-4"></p>
               )}

              <h4 className="text-lg font-semibold mt-4"> 🛍️ Items Ordered:</h4>
              <ul className="mt-2 space-y-2">
                {order.items.map((item, idx) => (
                  <li key={idx} className="flex items-center justify-between p-4 border rounded-lg shadow-sm">
